...
int main(int argc, char **argv) {
  if (argc != 3) {
    fprintf(stderr, "Need 3 arguments to run the server");
    return EXIT_FAILURE;
  }
  ...
  if (lflags >= 0) {
    fcntl(listen_fd, F_SETFL, lflags | O_NONBLOCK);
  }
    ...
    if (first_type1_time != 0) {
      ...
      if (type1_count < expected_clients && elapsed >= 1) {
        ...
        for (int j = 0; j < FD_SETSIZE; j++) {
          if (clients[j].fd != -1) {
            ...
            if (clients[j].out_head > 0) {
              ...
              memmove(clients[j].outbuf,
                      clients[j].outbuf + clients[j].out_head, rem);
        ...
        while (finishing) {
          ...
          if (select(fdmax + 1, NULL, &wfds, NULL, &flush_tv) <= 0)
            break;
          finishing = 0;
          for (int k = 0; k < FD_SETSIZE; k++) {
            ...
            if (FD_ISSET(wfd, &wfds)) {
              ...
              if (tosend > 0) {
                ssize_t s = send(wfd, clients[k].outbuf + clients[k].out_head,
                                 tosend, 0);
                if (s > 0) {
                  ...
                  FD_CLR(wfd, &master_write);
                  if (clients[k].outbuf) {
                    free(clients[k].outbuf);
                    clients[k].outbuf = NULL;
                  }
                  clients[k].fd = -1;
                  clients[k].alive = 0;
                } else {
        ...
        for (int j = 0; j < FD_SETSIZE; j++) {
          if (clients[j].fd != -1)
            close(clients[j].fd);
          if (clients[j].outbuf) {
            free(clients[j].outbuf);
            clients[j].outbuf = NULL;
          }
          return 0;
        }
      }
      ...
      for (int i = 0; i < FD_SETSIZE; i++) {
        ...
        if (FD_ISSET(wfd, &writefds)) {
          ...
          if (tosend > 0) {
            ssize_t s =
                send(wfd, clients[i].outbuf + clients[i].out_head, tosend, 0);
            if (s > 0) {
      ...
      for (int i = 0; i < FD_SETSIZE; i++) {
        ...
        ssize_t r = recv(fd, clients[i].buf + clients[i].buflen,
                         sizeof(clients[i].buf) - clients[i].buflen - 1, 0);
        int peer_closed = 0;
        if (r < 0) {
          if (errno == EINTR)
            continue;
          ...
          if (clients[i].buf[p] == '\n') {
            ...
            if (msglen >= 1) {
              ...
              if (type == 0) {
                ...
                uint32_t port_n = clients[i].addr.sin_port;
                memcpy(out + 1, &ip_n, 4);
                memcpy(out + 1 + 4, &port_n, 2);
                memcpy(out + 1 + 4 + 2, clients[i].buf + processed + 1,
                       payload_len);
                size_t out_len = 1 + 4 + 2 + payload_len;

                // enqueue to all alive clients' outbuf (non-blocking)
                for (int j = 0; j < FD_SETSIZE; j++) {
                  if (clients[j].fd != -1 && clients[j].alive) {
                    ...
                    if (avail >= out_len) {
                      memcpy(clients[j].outbuf + clients[j].out_tail, out,
                             out_len);
                      ...
                      // if it can't be enqueued, drop the client to avoid
                      // blocking
                      close(clients[j].fd);
                ...
                if (!clients[i].got_type1) {
                  ...
                  inet_ntop(AF_INET, &clients[i].addr.sin_addr, ipbuf,
                            sizeof(ipbuf));
                  fprintf(stderr,
                          "[server] Received type1 from %s: %u -> count=%d\n",
                          ipbuf, ntohs(clients[i].addr.sin_port), type1_count);
                  if (first_type1_time == 0)
                    first_type1_time = time(NULL);
                }
                ...
                if (type1_count >= expected_clients) {
                  ...
                  for (int j = 0; j < FD_SETSIZE; j++) {
                    if (clients[j].fd != -1) {
                      ...
                      if (avail >= 2) {
                        memcpy(clients[j].outbuf + clients[j].out_tail, tout,
                               2);
                        clients[j].out_tail += 2;
                        FD_SET(clients[j].fd, &master_write);
                        if (clients[j].fd > fdmax)
                  ...
                  fprintf(
                      stderr,
                      "[server] Broadcasting type 1 to %d clients (expected "
                      "%d)\n",
                  ...
                  while (finishing) {
                    ...
                    wfds = master_write;
                    struct timeval flush_tv = {1, 0};
                    if (select(fdmax + 1, &rfds, &wfds, NULL, &flush_tv) <= 0)
                      break;
                    finishing = 0;
                    for (int k = 0; k < FD_SETSIZE; k++) {
                      if (clients[k].fd == -1)
                      ...
                      if (FD_ISSET(wfd, &wfds)) {
                        size_t tosend =
                            clients[k].out_tail - clients[k].out_head;
                        if (tosend > 0) {
                          ssize_t s =
                              send(wfd, clients[k].outbuf + clients[k].out_head,
                                   tosend, 0);
                          if (s > 0) {
                            clients[k].out_head += s;
                            if (clients[k].out_head == clients[k].out_tail) {
